...
format:
  dime-revealjs:
    ...
    output-file: apresentacao_cras.html
    toc: true
    toc_float: true
    toc-depth: 2
    toc-location: body
...
-   O problema é confirmado por profissionais de linha de frente? O que eles pensam da proposta de solução?

-   Onde funciona bem um sistema de agendamento e fornecimento de
    informações automatizado?

    -   Se já foi tentado no SUAS, por que não funcionou? Ou por que não
        foi escalado de forma nacional?

    -   O que impediria de testar em 1 CRAS piloto?
